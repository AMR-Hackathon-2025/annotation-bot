...
def parse_metadata(lines):
    ...
    for line in lines:
        if line.startswith('# Annotated with'):
            metadata['sample_id'] = line.replace('# Annotated with', '').strip()
...
def parse_tsv_file(file_path):
    ...
    # Parse column header (should be the last line starting with '#')
    column_header = header_lines[-1].strip('#').strip().split('\t')
...
def import_to_database(conn, metadata, annotations):
    ...
    # Insert metadata and get its ID
    cursor.execute("""
        INSERT INTO metadata (sample_id, software_version, database_version, database_type, doi, url, file_path)
    ...
    for i in range(0, len(annotations), batch_size):
        ...
        query = f"""
            ...
            (metadata_id, sequence_id, feature_type, start_position, stop_position, 
...
def main():
    ...
    parser.add_argument('--db-name', default='bakta_tsv', help='PostgreSQL database name')
